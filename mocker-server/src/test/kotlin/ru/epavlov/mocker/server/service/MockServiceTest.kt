...
class MockServiceTest : BaseTest() {
    ...
    fun checkFindByQueryParams() {
        ...
        val found = service.getResponse(
            MockRequest(
                path,
                method,
                mapOf("sessionId" to listOf(sessionId), "contactId" to listOf(contactId)),
                emptyMap()
            )
        )
    ...
    fun notFoundParamTest2() {
        ...
        var r1 = service.create(mock)
        var found = service.getResponse(
            MockRequest(
                path,
                method,
                mapOf("sessionId" to listOf(UUID.randomUUID().toString()), "contactId" to listOf(contactId)),
                emptyMap()
            )
        )
        assert(found == null)
        found = service.getResponse(
            MockRequest(
                path,
                method,
                mapOf("sessionId" to listOf(sessionId), "contactId" to listOf(UUID.randomUUID().toString())),
                emptyMap()
            )
        )
        assert(found == null)
